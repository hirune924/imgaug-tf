...
def random_color(image, alpha_range=(0.2, 0.8), prob=0.5):
    ...
    alpha = tf.random.uniform(
        [], minval=alpha_range[0], maxval=alpha_range[1], dtype=tf.float32
    )
...
def random_rotate(
    ...
    degree = tf.random.uniform(
        [], minval=degree_range[0], maxval=degree_range[1], dtype=tf.float32
    )
    return apply_func_with_prob(
        F.rotate,
        image,
...
def random_translate_x(image, percent=0.5, replace=0, prob=0.5):
    ...
    pixels = tf.cast(image_width, tf.float32) * tf.random.uniform(
        [], minval=-percent, maxval=percent, dtype=tf.float32
    )
...
def random_translate_y(image, percent=0.5, replace=0, prob=0.5):
    image_height, image_width = tf.shape(image)[0], tf.shape(image)[1]
    pixels = tf.cast(image_height, tf.float32) * tf.random.uniform(
        [], minval=-percent, maxval=percent, dtype=tf.float32
    )
...
def random_hsv_in_yiq(
    ...
    return apply_func_with_prob(
        ...
        image,
        (max_delta_hue, lower_saturation, upper_saturation, lower_value, upper_value),
        prob,
    )


def random_gaussian_filter2d(image, filter_shape_range=(3, 7), prob=0.5):
    ksize = tf.random.uniform(
        [], minval=filter_shape_range[0], maxval=filter_shape_range[1], dtype=tf.int32
    )
    # sigma = 0.3*((tf.cast(ksize, tf.float32)-1)*0.5 - 1) + 0.8
    return apply_func_with_prob(
        tfa.image.gaussian_filter2d, image, ([ksize, ksize], 1.0), prob
    )
...
def random_resized_crop(
    ...
    def _random_resized_crop(
        image, size, area_range=(0.05, 1.0), aspect_ratio_range=(0.75, 1.33)
    ):
        image = F.random_crop(
            image, area_range=area_range, aspect_ratio_range=aspect_ratio_range
        )
    ...
    return tf.cond(
        ...
        lambda: _random_resized_crop(
            image, size, area_range=area_range, aspect_ratio_range=aspect_ratio_range
        ),
...
def random_cutout(image, num_holes=8, hole_size=20, replace=0, prob=0.5):
    ...
    return apply_func_with_prob(
        _random_cutout, image, (num_holes, hole_size, replace), prob
    )
...
def random_zoom(
    ...
    return apply_func_with_prob(
        F.scale_xy, image, ((scale, scale), interpolation, fill_mode, fill_value), prob
    )
